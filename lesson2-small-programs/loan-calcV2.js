...
function calc(loanAmt, monthlyRate, loanDur) {
  if (monthlyRate === 0) {
...
function changeOrNewProfile() {
  ...
  if (changeOrNewProfile === 'new') {
    while (true) {
      start();
      let mPay = calc(userInfo.loan, userInfo.monthlyR, userInfo.duration);
      monthPayDisplay(mPay);
...
function anotherChange() {
  ...
  if (moreChanges !== 'y' && moreChanges !== 'n') {
    ...
    monthPayDisplay(calc(userInfo.loan, userInfo.monthlyR, userInfo.duration));
    repeat();
  }
}
