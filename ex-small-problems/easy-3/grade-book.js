function getGrade(firstScore, secScore, thirdScore) {
  const gradeBoundaries = { A: 90, B: 80, C: 70, D: 60, F: 0 };
  let finalGrade;
  let average = Math.round((firstScore + secScore + thirdScore) / 3);
  for (let grade in gradeBoundaries) {
    if (average >= gradeBoundaries[grade]) {
      finalGrade = grade;
      break;
    }
  }
  return finalGrade;
}
/*
Write a function that determines the mean (average)
of the [three scores] passed to it,
and returns the letter associated with that grade.

Numerical score letter grade list:

90 <= score <= 100: 'A'
80 <= score < 90: 'B'
70 <= score < 80: 'C'
60 <= score < 70: 'D'
0 <= score < 60: 'F'


Tested values are all between 0 and 100.
There is no need to check for negative values or values greater than 100.
*/

console.log(getGrade(95, 90, 93));
console.log(getGrade(50, 50, 95));
